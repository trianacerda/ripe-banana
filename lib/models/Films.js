...
module.exports = class Films {
  ...
  static async getFilmById(id) {
    const { rows } = await pool.query(
      'SELECT films.id, title, released FROM films WHERE id = $1',
      [id]
    );
    const studio = await pool.query(
      'SELECT studios.id, studios.name FROM studios LEFT JOIN films ON studios.id = films.id WHERE films.id = $1',
      [id]
    );
